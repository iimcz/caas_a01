...
def __main__():
    ...
    topic_names = {}
    for id in to_send:
        topic_names[id] = [to_send[id].lower().replace(' ', '-'), 'art01/' + to_send[id].lower().replace(' ', '-')]

    client = mqtt.Client('cad_sender-1', True)
    client.tls_set()
    client.username_pw_set('cad_pub', 'C#da--t312')
    client.enable_logger()

    while True:
        cret = client.connect(host='green.iim.cz', port=8883)
        next_call = time.time()
